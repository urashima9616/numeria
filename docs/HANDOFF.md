...
> 日期:2026-09-11。本文档面向接手开发的 agent/工程师,涵盖项目现状、架构约定、踩坑记录与后续路线。
...
- 森林前三个符文依次对应萤火灯、藤桥、镜像林地。完成全部解锁守护者，宝箱改为可选；旧版已解锁的 Boss 不重新上锁。
- `SpellSequence` 为火焰/水浪/镜像三类技能提供独立时间轴，`SpellTrace` 携带实际解题数据。伤害在 1.22 秒命中点结算一次；其他技能仍走旧系统。
- 存档升级 v10：森林宝箱使用稳定 ID，同时保留旧坐标别名；测试没有读取或写入 Lucas 的真实进度。
